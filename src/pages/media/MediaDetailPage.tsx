...
const MediaDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const mapRowToItem = (row: any) => ({
      ...
      provider: { name: row.provider_name || 'DTMI', logoUrl: row.provider_logo_url || null },
      ...
      eventRegistrationInfo: (row as any).event_registration_info || (row as any).registration_url || null,
      ...
      try {
        ...
        try {
          ...
          if (data && data.length > 0) {
            const mapped = mapRowToItem(data[0])
            setItem(mapped)
    ...
    const fetchProviderPosts = async () => {
      ...
      try {
        ...
        const posts = (rel || []).map((r) => ({
          ...
          provider: { name: (r as any).provider_name || 'DTMI', logoUrl: (r as any).provider_logo_url || null },
                ...
                <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 mb-6">
                  ...
                  <ul className="space-y-2">
                    ...
                    <li className="flex items-start">
                      <div className="text-blue-600 mr-2">?</div>
                      <span className="text-gray-700">
                        <strong>Digital Leadership: </strong> Anchored in strategic vision for innovation and digital transformation.
                      </span>
                    </li>
                    <li className="flex items-start">
                      <div className="text-blue-600 mr-2">?</div>
  ...
  const getAdditionalProps = (item: any) => {
    ...
    switch (type) {
      ...
      case 'tool':
        ...
        if (item.date) metadata.lastUpdated = formatDate(item.date)
        break
      case 'announcement':
        if (item.provider?.name) metadata.source = String(item.provider.name)
        if (item.date) metadata.date = formatDate(item.date)
  ...
  // Remove the loading screen - show content immediately for better UX
  // if (loading) {
  //   return (
          ...
          <div className="container mx-auto px-4 py-8 max-w-7xl">
            ...
            <div className="w-full">
              ...
              <div className="">
                ...
                <div className="bg-gray-50 border border-gray-100 rounded-lg p-3 mb-3 flex items-center">
                  {item.provider?.logoUrl && (
                    <img
                      src={item.provider.logoUrl}
                      alt={item.provider.name}
                    ...
                    {item?.imageUrl && (
                      <div className="w-full mb-4">
                        <img
                          src={item.imageUrl}
                          alt={item.title}
                          className="w-full h-56 sm:h-64 md:h-80 object-cover rounded-xl shadow"
                          loading="eager"
                        />
                      </div>
                    )}
