-- Complete Blog Table Setup for End-to-End Blog Form Support
-- This script creates the blogs table and adds all necessary fields

-- ============================================================================
-- STEP 1: Add Blog-Specific Fields to media_items Table
-- ============================================================================
...
-- ============================================================================
-- STEP 2: Create Blogs Table (Following the Same Pattern as Articles, Videos, etc.)
-- ============================================================================

-- Create the blogs table with 1:1 relationship to media_items
...
-- ============================================================================
-- STEP 3: Update Views to Include Blog Data (Following Existing Pattern)
-- ============================================================================

-- Update the main view to include blog-specific fields (following the existing pattern)
...
SELECT
  ...
  -- Blog fields (following the same pattern as article fields)
  ...
  -- General body convenience (prioritize blog, then article, following existing pattern)
  ...
  e.registration_url,
  e.timezone,
  -- Common filter fields used by app (existing, pass-through if present)
...
-- Recreate the public view (existing pattern)
...
-- Create permissive policies for development (matching existing pattern)
...
-- Function to create a complete blog post (following the existing create_media_item pattern)
...
-- Verify new columns were added to media_items
SELECT 'media_items blog columns added successfully' as status, 
       count(*) as new_columns
FROM information_schema.columns 
WHERE table_name = 'media_items' 
  AND column_name IN ('hero_image', 'category', 'featured', 'read_time', 'highlights');

-- Verify blogs table was created
...
-- Verify the view includes blog fields
SELECT 'v_media_all view updated with blog support' as status
WHERE EXISTS (
  SELECT 1 FROM information_schema.columns 
  WHERE table_name = 'v_media_all' 
    AND column_name IN ('blog_body_html', 'author_name')
);

-- Final success message
SELECT '🎉 Blog table setup completed! Your blog form should now work end-to-end with the separate blogs table.' as final_status;