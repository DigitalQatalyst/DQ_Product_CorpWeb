...
,
  ...
  const pillItems = badges.slice(0, 3).map((badge, index) => ({
    text: badge,
    variant: (index === 0 ? 'primary' : index === 1 ? 'info' : 'success') as const,
    ariaPrefix: index === 0 ? 'Type' : index === 1 ? 'Category' : 'Business Stage',
  }))
  ...
  return (
    ...
    >
      ...
      >
        ...
        {hasPills && (
          <div className="mb-2 flex-shrink-0 static">
            <div className="flex flex-wrap gap-2 static">
              {pillItems.map((pill, index) => (
                <span
                  key={index}
                  className={`inline-flex items-center ${designTokens.spacing.pill.padding} rounded-full text-xs font-medium whitespace-nowrap static z-auto ${tagVariants[pill.variant]}`}
                  tabIndex={0}
        ...
        <div className="mb-2 flex-shrink-0">
          ...
          {(metadata?.date || metadata?.updated) && (
            <div className={`${designTokens.typography.metadata.size} ${designTokens.typography.metadata.color} flex items-center gap-2`}>
              <Calendar size={12} aria-label="Published date icon" />
              <span aria-label={`${metadata?.updated ? 'Updated' : 'Published'}: ${metadata?.updated || metadata?.date}`}>
                {metadata?.updated ? `Updated ${metadata.updated}` : metadata?.date}
              </span>
            </div>
          )}
          {/* Row 2: Provider/Author chain */}
          {(metadata?.author || metadata?.speaker) && (
            <div className={`${designTokens.typography.metadata.size} ${designTokens.typography.metadata.color} flex items-center gap-2 mt-1 min-w-0`}>
              <Building size={12} aria-label="Provider icon" />
              <span
