...
interface ModernDQChatbotProps {
...
const ModernDQChatbot: React.FC<ModernDQChatbotProps> = ({ initialMessage, onMessageProcessed }) => {
  ...
  useEffect(() => {
    if (initialMessage && initialMessage.trim() && processedInitialMessage.current !== initialMessage) {
      ...
      if (onMessageProcessed) {
        setTimeout(() => onMessageProcessed(), 100);
      }
    }
  ...
  const handleSendMessage = async (message?: string) => {
    ...
    try {
      ...
      console.error('ModernDQChatbot error:', error);
                ...
                <div key={message.id} className="message-container">
                  {message.role === 'user' ? (
                    <div className="flex justify-end">
                      <div className="bg-primary text-white rounded-2xl rounded-br-sm px-4 py-3 max-w-[70%] shadow-sm">
        ...
        <div className="border-t border-gray-100 px-6 py-4 bg-white">
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleSendMessage(); }} className="relative mb-4">
            <input
              ...
              className="w-full px-5 py-4 pr-14 border-2 border-primary/30 rounded-xl focus:outline-none focus:border-primary text-sm transition-colors"
            />
            <button
              type="submit"
              disabled={!inputMessage.trim()}
              className="absolute right-3 top-1/2 transform -translate-y-1/2 p-2 text-primary hover:text-primary-600 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
            ...
            >
              {TRENDING_QUESTIONS.map((question, index) => (
                <button
                  ...
                  className="flex-shrink-0 px-4 py-2 bg-white border border-gray-200 rounded-full text-sm text-gray-700 hover:border-primary hover:text-primary transition-colors whitespace-nowrap"
                >
                  {question}
                </button>
              ))}
...
export default ModernDQChatbot;
