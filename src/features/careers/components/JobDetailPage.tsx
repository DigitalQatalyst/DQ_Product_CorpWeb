...
import { Briefcase, MapPin, Clock, ArrowLeft, CheckCircle, Building2, Calendar, Share2, Loader } from "lucide-react";
...
export function JobDetailPage({ jobId }: { jobId: string }) {
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({ title: `${job?.title} at DigitalQatalyst`, url: window.location.href });
    } else {
  ...
  const metaItems = [
    ...
    { icon: Calendar, label: "Posted Date", value: new Date(job.postedDate).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" }) },
  ];
    ...
    <div className="min-h-screen bg-muted/30">
      ...
      <div className="bg-background border-b border-border">
        <div className="container mx-auto px-4 py-4">
          <Breadcrumb className="mb-2">
            <BreadcrumbList>
              <BreadcrumbItem><BreadcrumbLink render={<Link href="/careers" />}>Careers</BreadcrumbLink></BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbLink render={<Link href="/jobs" />}>Job Listings</BreadcrumbLink></BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbPage className="truncate max-w-[200px]">{job.title}</BreadcrumbPage></BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
          ...
          <div className="flex flex-col lg:flex-row items-start justify-between gap-8">
            <div className="lg:w-2/3">
              <div className="flex items-center gap-3 mb-4 flex-wrap">
                <h1 className="text-4xl font-bold text-foreground leading-tight">{job.title}</h1>
                <Badge variant="outline" className="bg-accent/10 text-accent border-accent/20">Open</Badge>
              </div>
              <p className="text-muted-foreground font-medium mb-4">
                {job.department} • {job.location} • {job.type}
                {job.openPositions && ` • ${job.openPositions} Position${job.openPositions !== 1 ? "s" : ""} Available`}
              </p>
      ...
      <div className="container mx-auto px-4 py-12 max-w-4xl">
        ...
        <section className="mb-12">
          <h2 className="text-3xl font-bold text-foreground mb-6">Key Responsibilities</h2>
          <Card className="py-0 gap-0">
        ...
        {job.skills && (
          <section className="mb-12">
            <h2 className="text-3xl font-bold text-foreground mb-6">Required Skills & Competencies</h2>
            <div className="space-y-6">
              <Card className="py-0 gap-0">
                <CardContent className="p-8">
                  <h3 className="text-xl font-bold text-foreground mb-4">Core Skills</h3>
              ...
              <Card className="py-0 gap-0">
                <CardContent className="p-8">
                  <h3 className="text-xl font-bold text-foreground mb-4">Behavioral & Professional Competencies</h3>
                  <ul className="space-y-3">
