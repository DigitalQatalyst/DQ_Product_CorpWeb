...
import { Briefcase, MapPin, Clock, ArrowRight, Search, X, Filter, ChevronDown, Loader } from "lucide-react";
...
function FilterGroups({ jobs, filters, onChange, onReset }: {
  jobs: JobListing[];
  filters: Filters;
  ...
  const groups: { key: keyof Filters; label: string; options: string[] }[] = [
    { key: "department", label: "Department", options: [...new Set(jobs.map((j) => j.department))].sort() },
    { key: "location", label: "Location", options: [...new Set(jobs.map((j) => j.location))].sort() },
    { key: "type", label: "Work Type", options: [...new Set(jobs.map((j) => j.type))].sort() },
  ];
  return (
    <div className="space-y-1">
        ...
        <Collapsible key={g.key} defaultOpen>
          <div className="py-3 border-b border-border last:border-0">
            <CollapsibleTrigger className="flex w-full justify-between items-center text-sm font-semibold text-foreground mb-2">
              {g.label} <ChevronDown size={14} className="text-muted-foreground" />
            </CollapsibleTrigger>
            <CollapsibleContent>
              <div className="space-y-2 mt-1">
                {g.options.map((opt) => (
                  <div key={opt} className="flex items-center gap-2">
                    <Checkbox id={`${g.key}-${opt}`} checked={filters[g.key].includes(opt)} onCheckedChange={() => onChange(g.key, opt)} />
                    <Label htmlFor={`${g.key}-${opt}`} className="text-sm font-normal cursor-pointer">{opt}</Label>
                  </div>
                ))}
              </div>
            </CollapsibleContent>
      ...
      <button onClick={onReset} className="w-full mt-2 py-2 text-sm text-destructive hover:bg-destructive/10 rounded-lg transition-colors">
        Reset Filters
      </button>
    </div>
  );
    ...
    <Card className="hover:shadow-md transition-shadow flex flex-col">
      <CardContent className="p-6 flex flex-col flex-1">
        <div className="flex items-start justify-between gap-3 mb-3">
          <h3 className="text-lg font-semibold text-foreground leading-tight line-clamp-2 flex-1">{job.title}</h3>
          <Badge variant="outline" className="text-xs bg-accent/10 text-accent border-accent/20 shrink-0">Open</Badge>
        </div>
        ...
        <p className="text-sm text-muted-foreground line-clamp-3 leading-relaxed mb-4 flex-1">{job.description}</p>
...
export function JobsListingPage() {
  ...
  const [filters, setFilters] = useState<Filters>({ department: [], location: [], type: [] });
  ...
  const resetFilters = () => { setFilters({ department: [], location: [], type: [] }); setSearch(""); };
  ...
  const filtered = useMemo(() =>
    jobs.filter((j) => {
      ...
      if (filters.department.length && !filters.department.includes(j.department)) return false;
  ...
  const sidebar = <FilterGroups jobs={jobs} filters={filters} onChange={handleFilterChange} onReset={resetFilters} />;
    ...
    <div className="min-h-screen bg-muted/30">
      <div className="container mx-auto px-4 py-8">
        <Breadcrumb className="mb-6">
          <BreadcrumbList>
            <BreadcrumbItem>
              <BreadcrumbLink render={<Link href="/careers" />}>Careers</BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator />
            <BreadcrumbItem><BreadcrumbPage>Job Listings</BreadcrumbPage></BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>

        <h1 className="text-3xl md:text-4xl font-bold text-foreground mb-3">Open Positions</h1>
