...
import type { ProductCtaType, ProductDetail } from "@/features/products/hooks/useProducts";
    ...
    <>
      <div className="container mx-auto px-4 md:px-6 py-8">
        <div className="pl-4">
          <nav aria-label="Breadcrumb" className="flex items-center gap-1 text-sm text-muted-foreground">
            ...
            <Link href="/products/marketplace" className="hover:text-foreground transition-colors">Products</Link>
            <ChevronRight size={14} />
          ...
          <div className="pl-4">
            <div className="flex flex-col lg:flex-row items-start gap-12">
              <div className="lg:w-2/3">
                ...
                <p className="text-lg text-muted-foreground mb-10 leading-relaxed">{product.description}</p>
      ...
      <div className="container mx-auto px-4 md:px-6 py-16 md:py-20">
        <div className="pl-4 space-y-16 md:space-y-20">
          <section>
            <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-8">Key Features</h2>
          ...
          {detail.aboutParagraphs.length > 0 && (
            <section className="bg-muted/40 rounded-xl p-8 md:p-10 border border-border">
              <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-6">About {product.code}</h2>
          ...
          <section className="space-y-12">
            {detail.problemStatement && (
              <div>
                <h3 className="text-2xl md:text-3xl font-bold text-foreground mb-4">The Problem Space</h3>
                <p className="text-muted-foreground leading-relaxed text-base">{detail.problemStatement}</p>
              </div>
            )}
            {detail.solutionStatement && (
              <div>
                <h3 className="text-2xl md:text-3xl font-bold text-foreground mb-4">Our Unique Solution</h3>
                <p className="text-muted-foreground leading-relaxed text-base">{detail.solutionStatement}</p>
              </div>
            )}
            {detail.capabilities.length > 0 && (
              <div>
                <h3 className="text-2xl md:text-3xl font-bold text-foreground mb-6">{detail.capabilitiesLabel}</h3>
                <div className="space-y-4">
                  {detail.capabilities.map((cap) => (
                    <div key={cap.title} className={`border-l-4 p-6 rounded-r-lg ${cap.accent === "primary" ? "border-primary bg-primary/5" : "border-secondary bg-secondary/5"}`}>
                      <h4 className="font-bold text-foreground mb-2 text-lg">{cap.title}</h4>
            ...
            {detail.practicalValues.length > 0 && (
              <div>
                <h3 className="text-2xl md:text-3xl font-bold text-foreground mb-6">Practical Value</h3>
                <Card>
                  <CardContent className="p-0 divide-y divide-border">
                    {detail.practicalValues.map((pv) => (
                      <div key={pv.title} className="flex items-center gap-6 p-8">
                        ...
                        <div>
                          <h4 className="font-bold text-foreground text-lg mb-2">{pv.title}</h4>
          ...
          <section>
            <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-8">Product Category</h2>
              ...
              <div className="flex flex-col md:flex-row items-start gap-8">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-6">
                    <Badge variant="outline" className="text-base px-4 py-1.5 font-semibold">{product.category}</Badge>
                    <Separator className="flex-1" />
                  </div>
                  <p className="text-muted-foreground leading-relaxed text-base mb-8">
                    This product belongs to the {product.category} category, providing specialized solutions for organizations looking to enhance their digital transformation capabilities.
                  </p>
