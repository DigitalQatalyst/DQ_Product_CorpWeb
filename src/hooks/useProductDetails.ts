...
export function useProductDetails({
  ...
  // Query partner details via getProductWithPartner (non-courses)
  const {
    data: partnerData,
    error: partnerError,
    loading: partnerLoading,
    refetch: refetchPartner,
  } = useQuery(GET_PARTNER as any, {
  ...
  useEffect(() => {
    ...
    const partner = (partnerData as any)?.getProductWithPartner?.partner;
    ...
    if (marketplaceType !== "courses") {
      ...
      const enrichedProvider = {
        ...currentProvider,
        // Only override name if partner.name is a non-empty string
        name:
          typeof partner?.name === "string" && partner.name.trim() !== ""
            ? partner.name
            : currentProvider.name,
        shortDescription:
          typeof partner?.shortDescription === "string" && partner.shortDescription.trim() !== ""
            ? partner.shortDescription
            : currentProvider.shortDescription,
        emailAddress:
          typeof partner?.emailAddress === "string" && partner.emailAddress.trim() !== ""
            ? partner.emailAddress
            : currentProvider.emailAddress,
        companyName:
          typeof partner?.companyName === "string" && partner.companyName.trim() !== ""
            ? partner.companyName
            : currentProvider.companyName,
        location:
          typeof partner?.location === "string" && partner.location.trim() !== ""
            ? partner.location
            : currentProvider.location,
        industry:
          typeof partner?.industry === "string" && partner.industry.trim() !== ""
            ? partner.industry
            : currentProvider.industry,
        partnerType:
          typeof partner?.partnerType === "string" && partner.partnerType.trim() !== ""
            ? partner.partnerType
            : currentProvider.partnerType,
        website:
          typeof partner?.website === "string" && partner.website.trim() !== ""
            ? partner.website
            : currentProvider.website,
        publicServicesCount:
          typeof partner?.publicServicesCount === "number"
            ? partner.publicServicesCount
            : currentProvider.publicServicesCount,
      } as any;
      merged.provider = enrichedProvider;
    } else {
      // Ensure provider from mapped data is always used for courses
  ...
  }, [productData, courseData, partnerData, itemId, marketplaceType, shouldTakeAction]);
  // Expose a unified loading/error/refetch
  const loading = marketplaceType === "courses" ? courseLoading : (productLoading || partnerLoading);
  const error = (marketplaceType === "courses" ? courseError : (productError || partnerError)) as any;
  const refetch = marketplaceType === "courses" ? refetchCourse : async () => {
    await Promise.allSettled([refetchProduct?.(), refetchPartner?.()]);
  };
